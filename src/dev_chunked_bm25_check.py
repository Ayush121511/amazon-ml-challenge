"""Test chunked-streaming BM25 with name+address RRF fusion on the dev sample
-- the teammate's proven architecture, adapted to our BM25 scoring function.
Chunk size is set smaller than the teammate's 50k default so chunking
actually kicks in at dev-sample scale (real target pools are 150-270k rows).
"""
import sys
import time
...
import chunked_bm25 as cb

DEV = "data/processed/dev_sample"
TOP_K = int(sys.argv[1]) if len(sys.argv) > 1 else 75
QUERY_BATCH_SIZE = int(sys.argv[2]) if len(sys.argv) > 2 else 20000
...
def main():
    ...
    print(f"normalized in {time.time()-t0:.1f}s, query_batch_size={QUERY_BATCH_SIZE}, top_k={TOP_K}")
    ...
    print(f"\n--- BM25 name+address fusion (index-once, query-batched) vs S2 ---")
    t0 = time.time()
    cand2 = cb.build_candidates_fused(s1, s2, top_k=TOP_K, query_batch_size=QUERY_BATCH_SIZE, label="s2")
    print(f"built in {time.time()-t0:.1f}s")
    recall_of(cand2, true_s2, "Fused vs S2")

    print(f"\navg candidates per S1 (S2): {len(cand2)/len(s1):.1f}")

    if "--with-s3" in sys.argv:
        print(f"\n--- BM25 name+address fusion (index-once, query-batched) vs S3 ---")
        t0 = time.time()
        cand3 = cb.build_candidates_fused(s1, s3, top_k=TOP_K, query_batch_size=QUERY_BATCH_SIZE, label="s3")
        print(f"built in {time.time()-t0:.1f}s")
        recall_of(cand3, true_s3, "Fused vs S3")
